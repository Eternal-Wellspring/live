(function () {
  ...
  function toggleGreenRefs(t) {
    var id = sid(t.id);
    if (sid(refTopicId) === id) {
      refTopicId = "";
      openRef = "";
    } else {
      refTopicId = id;
      openRef = firstChapRef(t) || (refsFor(t)[0] || "");
    }
    paint();
  }
  ...
  function paint() {
    ...
    put(versesWrap, PAD, PAD, vsW0, vsH);
    versesWrap.style.height = vsH + "px";
    versesWrap.style.zIndex = "50";
    if (chart) {
    ...
    prevBtn.innerHTML = '<span class="ew-ch-lab">Prev</span>';
    ...
    nextBtn.innerHTML = '<span class="ew-ch-lab">Next</span>';
    ...
    versesWrap.style.setProperty("--fs", "13px");
    var need = Math.ceil(bar.scrollWidth + 20);
    if (need < 200) need = 200;
    vsW = userSize > 0 ? userSize : need;
    if (vsW < need) vsW = need;
    if (vsW > maxW) vsW = maxW;
    if (vsW < need && need <= maxW) vsW = need;
    ...
    var key = chapKey();
    var IND = Math.round(textSize("xx").w);
    var nameW, shown, refTopic, refList, wRef, cRef, parentBox, xVs, rightChap, rightWrap, rightHit;
    if (key !== verseChapKey) {
      verseChapKey = key;
      tealFlip = {};
    }
    ...
    if (openRef && refList.length) {
      rightHit = parseRefParts(openRef);
      rightChap = rightHit
        ...
        : (viewChap ? { abbr: viewChap.abbr, ch: viewChap.ch, num: viewChap.num } : null);
      if (rightChap && !rightChap.num && viewChap && rightChap.abbr === viewChap.abbr) rightChap.num = viewChap.num;
      if (rightChap && rightChap.num) {
        rightWrap = document.createElement("div");
        rightWrap.className = "tverse-wrap ref-vs";
        board.appendChild(rightWrap);
        var rW = Math.max(280, (chart ? chart.clientWidth : viewW) - xVs - PAD);
        var rH = Math.max(160, viewH - PAD * 2);
        put(rightWrap, xVs, PAD, rW, rH);
        ...
        rPrev.innerHTML = '<span class="ew-ch-lab">Prev</span>';
        ...
        rNext.innerHTML = '<span class="ew-ch-lab">Next</span>';
